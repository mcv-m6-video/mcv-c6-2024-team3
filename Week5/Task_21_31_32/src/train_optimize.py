...
def caller(config = None):
    with wandb.init(config=config):
        ...
        # Testing
        acc_train_final, loss_train_final, train_predictions, gt_labels_train = evaluate(model, loaders['validation'], loss_fn, args.device, description=f"Validation [Final]")
        ...
        with open(f'{nombre_run}/train_predictions.pkl', 'wb') as f:
        ...
        with open(f'{nombre_run}/gt_label_test.pkl', 'wb') as f:
            pickle.dump(gt_label_test, f)

        plot_label_accuracies(train_predictions, gt_labels_train, CLASS_NAMES, 'train', nombre_run)
        ...
        plt.savefig(f'{nombre_run}/loss.png')
        plt.cla()
        ...
        plt.ylabel('Accuracy')
        plt.legend(['Training', 'Validation'], loc='upper right')
        plt.savefig(f'{nombre_run}/accuracy.png')
        plt.cla()
...
if __name__ == "__main__":
    sweep_id = wandb.sweep(sweep_config, project="borrar")
