...
def caller(config = None):
    with wandb.init(config=config):
        ...
        config.clip_length = 4
        config.crop_size = 259
        config.nt = 1
        config.temporal_stride = 12
        ...
        # Testing
        acc_val_final, predictions_val, labels_val = evaluate(model, loaders['validation'], loss_fn, args.device, description=f"Validation [Final]")
        acc_test_final, predictions_test, labels_test = evaluate(model, loaders['testing'], loss_fn, args.device, description=f"Testing")
        ...
        with open(f'{folder_evaluation}/predictions_val.pkl', 'wb') as f:
            pickle.dump(predictions_val, f)

        with open(f'{folder_evaluation}/labels_val.pkl', 'wb') as f:
            pickle.dump(labels_val, f)

        with open(f'{folder_evaluation}/predictions_test.pkl', 'wb') as f:
            pickle.dump(predictions_test, f)
        
        with open(f'{folder_evaluation}/labels_test.pkl', 'wb') as f:
...
if __name__ == "__main__":
    ...
    if dataset_type == 31:
        sweep_config = sweep_config_31
    elif dataset_type == 32:
        sweep_config = sweep_config_32
    ...
    sweep_id = wandb.sweep(sweep_config, project=f'baseline_eval_{dataset_type}')
