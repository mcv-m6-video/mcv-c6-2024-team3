...
from datasets.HMDB51Dataset_4 import HMDB51Dataset, HMDB51Dataset_baseline
...
def train(
    ...
    for batch in pbar:
        ...
        clips_video = torch.tensor_split(clips, num_seg, dim=1)

        outputs=[]

        for clip in clips_video:
            output = model(clip.squeeze(1)) #[16, 51]
            outputs.append(output.unsqueeze(0)) #[3, 16, 51]
        
        final_outputs = torch.mean(torch.cat(outputs), dim=0)
        # Compute loss
    ...
    for batch in pbar:
        ...
        with torch.no_grad():
            if mode == 'BL':
                ...
                for clip in clips_video:
                    output = model(clip.squeeze(1)) #[16, 51]
                    outputs_list.append(output.unsqueeze(0)) #[3, 16, 51]
                
                outputs = torch.mean(torch.cat(outputs_list), dim=0)
...
def create_datasets(
    ...
    for regime in HMDB51Dataset.Regime:
        if mode == 'train':
            ...
            datasets[regime.name.lower()] = HMDB51Dataset_baseline(
                frames_dir,
                annotations_dir,
                split,
                regime,
...
def caller(config = None):
    with wandb.init(config=config):
        ...
        if not os.path.exists(os.path.join('results', nombre_run)):
            os.makedirs(f'results/{nombre_run}')

        # Create datasets
        datasets = create_datasets(
            frames_dir='/ghome/group03/C6/frames',
        ...
        model = model_creator.create('x3d_xs', False, datasets["training"].get_num_classes())
        ...
        for epoch in range(config.n_epochs):
            ...
            acc_train, loss_train = train(model, loaders['training'], optimizer, loss_fn, 'cuda', description=description, num_seg = config.num_seg)
            ...
            if epoch % config.validate_every == 0:
                ...
                acc_val, loss_val, _, _ = evaluate(model, loaders['validation'], loss_fn, 'cuda', description=description, mode = '', num_seg=config.num_seg)
        ...
        torch.save(model.state_dict(), f'results/{nombre_run}/model_default.pth')

        # Testing
        acc_train_final, loss_train_final, train_predictions, gt_labels_train = evaluate(model, loaders['validation'], loss_fn, 'cuda', description=f"Validation [Final]", mode = '', num_seg=config.num_seg)
        acc_test_final, loss_test_final, test_predictions, gt_label_test = evaluate(model, loaders['testing'], loss_fn, 'cuda', description=f"Testing", mode = '', num_seg=config.num_seg)
        ...
        with open(f'results/{nombre_run}/train_predictions.pkl', 'wb') as f:
            pickle.dump(train_predictions, f)
        
        with open(f'results/{nombre_run}/test_predictions.pkl', 'wb') as f:
            pickle.dump(test_predictions, f)

        with open(f'results/{nombre_run}/gt_labels_train.pkl', 'wb') as f:
            pickle.dump(gt_labels_train, f)

        with open(f'results/{nombre_run}/gt_label_test.pkl', 'wb') as f:
            pickle.dump(gt_label_test, f)


        plot_label_accuracies(train_predictions, gt_labels_train, CLASS_NAMES, 'train', f'results/{nombre_run}')
        plot_label_accuracies(test_predictions, gt_label_test, CLASS_NAMES, 'test', f'results/{nombre_run}')
        ...
        plt.savefig(f'results/{nombre_run}/loss.png')
        plt.cla()
        ...
        plt.ylabel('Accuracy')
        plt.legend(['Training', 'Validation'], loc='upper right')
        plt.savefig(f'results/{nombre_run}/accuracy.png')
        plt.cla()
...
if __name__ == "__main__":
    ...
    if torch.cuda.is_available():
        ...
        print("CUDA device name:", torch.cuda.get_device_name(0))  # Print the name of the GPU device
        print("CUDA device count:", torch.cuda.device_count())  # Print the number of available GPU devices
        print("CUDA device index in use:", torch.cuda.current_device())  # Print the index of the current GPU device in use
    else:
        print("CUDA is not available. You are using CPU.")

    sweep_id = wandb.sweep(sweep_config, project="task_4.1_good")
