...
class HMDB51Dataset(Dataset):
    ...
    def __getitem__(self, idx: int) -> tuple:
        ...
        modality_paths = sorted(glob(os.path.join(escape(modality_path), "*.jpg"))) # get sorted frame paths
        ...
        if modality_len <= self.clip_length * self.temporal_stride:
            # Not enough frames to create the clip
        ...
        # Read frames from the video with the desired temporal subsampling
        video_modality = None
        for i, path in enumerate(modality_paths[clip_begin:clip_end:self.temporal_stride]):
            frame = read_image(path)  # (C, H, W)
            if video_modality is None:
                video_modality = torch.zeros((self.clip_length, 3, frame.shape[1], frame.shape[2]), dtype=torch.uint8)
            video_modality[i] = frame
        #-----------------------------------------------------------

        # Get label from the annotation dataframe and make sure video was read
        label = df_idx['class_id']
        assert video_modality is not None
        assert video is not None

        return video, video_modality, label, video_path
    ...
    def collate_fn(self, batch: list) -> dict:
        ...
        transformed_video_modalities = [self.transform(clip).permute(1, 0, 2, 3) for clip in unbatched_video_modalities]
        ...
        batched_video_modalities = torch.cat([d.unsqueeze(0) for d in transformed_video_modalities], dim=0)

        return dict(
            clips=batched_clips, # (B, C, T, H, W)
            video_modalities=batched_video_modalities,
...
if __name__ == '__main__':
    ...
    dataloader = DataLoader(
            dataset,
            batch_size=16,
            shuffle=False,  # Shuffle only for training dataset
    ...
    for i in dataloader:
        clips = i['clips']
        video_modalities = i['video_modalities']
        labels = i['labels']
        ...
        for batch_idx in range(tensor.size(0)):
            for clip_idx in range(tensor.size(2)):
                # Select the image
                image = tensor[batch_idx, :, clip_idx]
                # Convert to numpy
                image = image.numpy()
                # Transpose the image
                ...
                # Display the image
                cv2.imwrite(f'clips/clip{batch_idx}_{clip_idx}.jpg', image * 255)

        reset_folder('clips_modality')
        tensor = video_modalities
        for batch_idx in range(tensor.size(0)):
            for clip_idx in range(tensor.size(2)):
                # Select the image
                image = tensor[batch_idx, :, clip_idx]
                ...
                image = cv2.cvtColor(image, cv2.COLOR_RGB2BGR)
                # Display the image
                cv2.imwrite(f'clips_modality/clip{batch_idx}_{clip_idx}.jpg', image * 255)
