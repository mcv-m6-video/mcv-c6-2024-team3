...
def caller(config = None):
    with wandb.init(config=config):
        ...
        if not os.path.exists(os.path.join('results_task2', nombre_run)):
            os.makedirs(f'results_task2/{nombre_run}')
        ...
        torch.save(model.state_dict(), f'results_task2/{nombre_run}/model_default.pth')
        ...
        with open(f'results_task2/{nombre_run}/train_predictions.pkl', 'wb') as f:
            pickle.dump(train_predictions, f)
        
        with open(f'results_task2/{nombre_run}/test_predictions.pkl', 'wb') as f:
            pickle.dump(test_predictions, f)

        with open(f'results_task2/{nombre_run}/gt_labels_train.pkl', 'wb') as f:
            pickle.dump(gt_labels_train, f)

        with open(f'results_task2/{nombre_run}/gt_label_test.pkl', 'wb') as f:
            pickle.dump(gt_label_test, f)


        plot_label_accuracies(train_predictions, gt_labels_train, CLASS_NAMES, 'validation', f'results_task2/{nombre_run}')
        plot_label_accuracies(test_predictions, gt_label_test, CLASS_NAMES, 'test', f'results_task2/{nombre_run}')
        plot_confusion_matrix(train_predictions, gt_labels_train, CLASS_NAMES, 'validation', f'results_task2/{nombre_run}')
        plot_confusion_matrix(test_predictions, gt_label_test, CLASS_NAMES, 'testing', f'results_task2/{nombre_run}')
        ...
        plt.savefig(f'results_task2/{nombre_run}/loss.png')
        plt.cla()
        ...
        plt.ylabel('Accuracy')
        plt.legend(['Training', 'Validation'], loc='upper right')
        plt.savefig(f'results_task2/{nombre_run}/accuracy.png')
        plt.cla()
...
if __name__ == "__main__":
    sweep_id = wandb.sweep(sweep_config, project="2_2_week7_new")
