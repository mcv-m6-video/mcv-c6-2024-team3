...
from datasets.HMDB51Dataset import HMDB51Dataset, HMDB51DatasetOF
...
def create_datasets(
    ...
    for regime in HMDB51Dataset.Regime:
        datasets[regime.name.lower()] = HMDB51DatasetOF(
            frames_dir,
            annotations_dir,
            split,
            regime,
...
def caller(config = None):
    with wandb.init(config=config):
        ...
        wandb.run.name = '2D_OF_3channels_0padding'
        wandb.run.save()
        ...
        datasets = create_datasets(
            frames_dir='/ghome/group03/C6/C6_optical_flow',
