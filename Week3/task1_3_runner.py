...
if __name__ == '__main__':
    ...
    path_saves = './tracking_block_mathing'
    ...
    num_files = len(files)
    optical_flow = "our_own"
    type_combi = "indiv"

    for type_op in ['indiv']:
        tracking = Tracking(
            pathDets = str(yolo_runs), 
            pathImgs = video_frames_tracking, 
            pathOutput = path_saves, 
            w = w, 
            h = h,
            num_files = num_files, 
            display = False, 
        ...
        os.rename('./tracking_block_mathing/tracking_OFSORT.txt', f'./tracking_block_mathing/tracking_OFSORT_{type_op}.txt')
        
        os.system(f'python3 plot_bb_trail.py tracking_block_mathing/tracking_OFSORT_{type_op}.txt ../OptionalTaskW2Dataset/train/S03/c010/vdo.avi tracking_block_mathing/video_SORTOF_{type_op}.mp4')